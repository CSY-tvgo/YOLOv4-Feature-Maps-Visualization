...
def draw_img(img, boxes, score, label, word_dict, color_table,):
    ...
    for i in range(len(boxes)):
        ...
        # draw box
        cv2.rectangle(img, (x_min, y_min), (x_max, y_max), curr_color, thickness=3)
        # draw font
        if word_dict is not None:
            text_name = "{}".format(word_dict[curr_label])
            cv2.putText(img, text_name, (x_min + 2, y_min + 25), font, 1, curr_color, thickness=2)
        if score is not None:
            text_score = "{:2d}%".format(int(score[i] * 100))
            cv2.putText(img, text_score, (x_min + 2, y_min), font, 1, curr_color, thickness=2)
    return img
